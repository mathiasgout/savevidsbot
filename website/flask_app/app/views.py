...
from app.outils import add_video_info_to_the_session, add_user_info_to_the_session, get_user_videos, user_deletion, video_deletion

from flask import Blueprint, render_template, redirect, url_for, session, request, current_app, flash, abort
...
def users(screen_name):
    ...
    if screen_name not in session["users"].keys():
        if add_user_info_to_the_session(session=session, screen_name=screen_name):
            user_videos = get_user_videos(session=session, screen_name=screen_name, page=page)
            if len(user_videos) < current_app.config["VIDEOS_PER_PAGE"]:
                last_page = True
            return render_template("users.html", screen_name=screen_name, user_videos=user_videos, page=page, last_page=last_page)
