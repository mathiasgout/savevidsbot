...
def add_video_info_to_the_session(session, video_id: str) -> bool:
    ...
    """
    try:
        video_document = get_document(collection_name=current_app.config["VIDEOS_COLLECTION"], document_name=video_id)
    ...
    if video_document:
        ...
        session["tweets"][video_id] = {"video_url":video_url, "creator":creator, "tweet_url":tweet_url, "thumbnail_url":thumbnail_url, "text":text}
...
def add_user_info_to_the_session(session, screen_name: str) -> bool:
    ...
    """
    try:
        user_document = get_document(collection_name=current_app.config["USERS_COLLECTION"], document_name=screen_name)
    except Exception as e:
        logger.error(f"Error occured : {e}")
        return abort(500)

    if user_document:
        requested_videos = [video["video_id"] for video in user_document["requested_videos"]]
    ...
    try:
        ...
        if user_document:
            ...
            for video_id in video_ids:
                delete_document(collection_name=current_app.config["VIDEOS_COLLECTION"], document_name=video_id)
            ...
            delete_document(collection_name=current_app.config["USERS_COLLECTION"], document_name=screen_name)

            # Flash du succès
            flash(f'User with screen name : "{screen_name}" banned with sucess!', category="sucess")
        
        else:
            flash(f'User with screen name : "{screen_name}" not found!', category="error")
...
def video_deletion(video_id: str):
    ...
    try:
        ...
        if video_document:
            ...
            delete_document(collection_name=current_app.config["VIDEOS_COLLECTION"], document_name=video_id)
            flash(f'Video with ID : "{video_id}" deleted with sucess!', category="sucess")
